...
regex="^(chore|fix|feat|int|integration|feature|bugfix|hotfix)\/[A-Za-z0-9\-]+$"
...
if [[ $branch_name =~ $regex ]]; then
  ...
  echo "Branch names should start with 'chore/', 'fix/', 'feat/', 'int/', 'integration/', 'feature/', 'bugfix/', or 'hotfix/' followed by a name."
  exit 1
fi
