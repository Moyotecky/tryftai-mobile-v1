        ...
        {expenseCategories.map((expenseCategory) => (
          <Card key={expenseCategory.category} style={{ width: 300 }}>
            <View
              ...
              <Image source={expenseCategory.icon} style={{ width: 30, height: 30 }} />
            </View>
            <Text className="text-2xl" weight="semi_bold">
              {expenseCategory.category}
            </Text>

            <Text className="mt-8 text-lg text-gray-600 " weight="semi_bold">
              SPENT
            </Text>
            <View className="flex-row items-end gap-2">
              ...
              <Text className="text-gray mb-1.5">out of {formatPrice(expenseCategory.budget)}</Text>
            </View>

            <ProgressBar
              progress={expenseCategory.amountSpent / expenseCategory.budget}
              className="mt-4"
            />
          </Card>
        ))}
